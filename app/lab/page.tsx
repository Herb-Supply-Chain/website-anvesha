...
export default function LabTestingPage() {
    ...
    const [activeTab, setActiveTab] = useState<'basic' | 'report'>('basic')
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="grid grid-cols-2 bg-gray-100 rounded-xl p-1">
                                {[
                                    ...
                                    { id: 'report', label: 'PDF Report Data' }
                                ].map((tab) => (
                                    <button
                                        key={tab.id}
                                        type="button"
                                        onClick={() => setActiveTab(tab.id as 'basic' | 'report')}
                                        className={`rounded-lg py-3 text-sm sm:text-base font-semibold transition-all ${
                                            activeTab === tab.id
                            ...
                            <p className="text-sm text-gray-500">
                                Use tabs to capture routine observations separately from the values that will appear in the PDF report.
                            </p>
                        </div>
                            ...
                            <>
                                ...
                                <div className="bg-gradient-to-br from-teal-50 to-white rounded-2xl p-6 sm:p-8 border-2 border-teal-100 shadow-sm">
                                    <div className="flex items-center gap-3 mb-6 pb-4 border-b-2 border-teal-100">
                                        ...
                                        <div>
                                            <h3 className="text-lg sm:text-xl font-bold text-gray-900">PDF Preview Snapshot</h3>
                                            <p className="text-sm text-gray-600">Quick view of what will be shown in the generated report.</p>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                        {[
                                            { title: 'Chemical Limits', rows: [
                                                { label: 'Lead (ppm)', value: formData.lead || '—', limit: 'Max 10 ppm' },
                                                { label: 'Arsenic (ppm)', value: formData.arsenic || '—', limit: 'Max 3 ppm' },
                                                { label: 'Cadmium (ppm)', value: formData.cadmium || '—', limit: 'Max 0.3 ppm' }
                                            ]},
                                            { title: 'Additional Chemicals', rows: [
                                                { label: 'Mercury (ppm)', value: formData.mercury || '—', limit: 'Max 1 ppm' },
                                                { label: 'Pesticide Residues', value: formData.pesticides || '—', limit: 'Below detection' },
                                                { label: 'Aflatoxins (ppb)', value: formData.aflatoxin || '—', limit: 'Max 20 ppb' }
                                            ]},
                                            { title: 'Microbial Profile', rows: [
                                                { label: 'Total Bacterial Count', value: formData.totalBacterialCount || '—', limit: '≤ 10^5 CFU/g' },
                                                { label: 'Yeast & Mold', value: formData.yeastMold || '—', limit: '≤ 10^3 CFU/g' },
                                                { label: 'Salmonella / E. coli', value: `${formData.salmonella || '—'} / ${formData.ecoli || '—'}`, limit: 'Absent' }
                                            ]}
                                        ].map((card) => (
                                            <div key={card.title} className="bg-white rounded-xl border border-teal-100 shadow-sm p-4 space-y-3">
                                                <div className="flex items-center justify-between">
                                                    <p className="text-sm font-semibold text-gray-900">{card.title}</p>
                                                    <span className="text-xs px-2 py-1 rounded-full bg-teal-100 text-teal-700 font-semibold">PDF</span>
                                                </div>
                                                <div className="space-y-2">
                                                    {card.rows.map((row) => (
                                                        <div key={row.label} className="flex items-center justify-between text-sm">
                                                            <div>
                                                                ...
                                                                <p className="text-xs text-gray-500">{row.limit}</p>
                                                            </div>
                                                            <p className="text-teal-700 font-semibold">{row.value}</p>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                    ...
                                    <div className="pt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                        <p className="text-sm text-gray-600">
                                            PDF preview uses the values you entered above. Click generate to download/print.
                                        </p>
                                        <button
                                            type="button"
                                            onClick={handlePrint}
                                            className="inline-flex items-center justify-center gap-2 px-4 py-3 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-xl shadow-sm transition-all"
                                        >
                                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-2H7a2 2 0 00-2 2v10a2 2 0 002 2h6m4-14v4m0 0l-2-2m2 2l2-2" />
                                            </svg>
                                            Generate PDF
                                        </button>
                                    </div>
